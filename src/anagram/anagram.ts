...
const letterExistsInWord = (word: string) => (letter: string) =>
  word.split('').some(isSameLetter(letter))

export const isSameWord = (a: string, b: string) =>
  a.length === b.length && a.split('').every(letterExistsInWord(b))

export const isSameWordCurry = (a: string) => (b: string) =>
  a.split('').every(letterExistsInWord(b))
...
export const checkAnagramWithHashmap = (wordA: string, wordB: string) => {
  ...
  wordA
    .split('')
    .forEach(letter =>
      characterCount.set(letter, (characterCount.get(letter) || 0) + 1),
    )

  wordB.split('').forEach(letter => {
    if (characterCount.has(letter) === false) {
      return false
    }

    characterCount.set(letter, characterCount!.get(letter) - 1)
  })
